...
- [Шаблон «Цепочки»  (для экспериментаторов)](#шаблон-цепочки--для-экспериментаторов)
...
## Шаблон «Цепочки»  (для экспериментаторов)
...
- Прокси типа AWG / WARP автоматически исключаются — они не поддерживают цепочки, но если они нужны какому-либо сервису, их можно выбрать в `MANUAL`.
- Расход памяти изза наложения подписок для сцепления может быть больше на 30-50% от того, сколько занял бы облегченный шаблон (до 80 МБ при подписках на ~300 URL)
